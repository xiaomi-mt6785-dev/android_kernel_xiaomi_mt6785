...
struct charger_ops {
	...
	int (*set_otg_current)(struct charger_device *dev, u32 uA);
...
extern int charger_dev_get_hvdcp_dpdm_status(
	struct charger_device *charger_dev, bool *status);
...
extern int charger_dev_get_ctd_dischg_status(struct charger_device *dev,
					     u8 *status);
extern int charger_dev_set_suspend(struct charger_device *dev, bool en);
